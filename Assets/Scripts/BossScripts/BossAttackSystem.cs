...
public class BossAttackSystem : MonoBehaviour
{
    ...
    IEnumerator SwordBarrage()
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            targetPos.y = data.initialPosition.y - 1.5f;
    ...
    IEnumerator GroundSpikesAttack()
    {
        ...
        {
            if (data.player == null) break;
            Vector3 target = data.player.position;
            ...
            if (data.playerRb != null && data.playerRb.linearVelocity.x != 0)
            {
                ...
                target += new Vector3(data.playerRb.linearVelocity.x * pred, 0, 0);
            }

            target.y = data.initialPosition.y - 1.5f;
            target.x = Mathf.Clamp(target.x, data.minArenaX, data.maxArenaX);
    ...
    IEnumerator SpawnSpike(Vector3 pos)
    {
        ...
        int hitCount = Physics2D.OverlapBoxNonAlloc(
            pos + Vector3.up * 1f, new Vector2(1.5f, 2f), 0f, data.hitBuffer
        );
        for (int i = 0; i < hitCount; i++)
        {
            if (data.hitBuffer[i].CompareTag("Player"))
                data.playerMainChar?.TakeDamage(geyserDamage);
        }
    }
    ...
    IEnumerator CrossPatternAttack()
    {
        ...
        Vector3 center = data.player.position;
        center.y = data.initialPosition.y - 1.5f;
        for (int i = -4; i <= 4; i++)
        {
            Vector3 pos = center + new Vector3(i * 2f, 0, 0);
            pos.x = Mathf.Clamp(pos.x, data.minArenaX, data.maxArenaX);
